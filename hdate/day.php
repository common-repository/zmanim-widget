...
$gmonth_t = date("m", $ts_t_day);
$gday_t = date("d", $ts_t_day);
$gyear_t = date("Y", $ts_t_day);
...
echo '<ul class="zmanim-inner-section"><li><b>'.$h_date_t.' ';
if ($hebrew) print translate_z('zmanim','hebrew');
echo '</b></li><li>';
echo '<ul id="zmanim-'.$ts_zero.'">';
//$zman="90";
if ( $zman_alot == "72" ) {
	...
	//$time_2 = date_sunrise($ts_t_day, SUNFUNCS_RET_STRING, $shlat, $shlong, 90+16.1, $shoffset);
	$time_ar = explode(':',date_sunrise($ts_t_day, SUNFUNCS_RET_STRING, $shlat, $shlong, 90+16.1, $shoffset));
	$hour = $time_ar[0];
	$min = '00';
...
'</li>';
list($hour,$min) = explode(':',date_sunrise($ts_t_day, SUNFUNCS_RET_STRING, $shlat, $shlong, 90+$zman_tallit, $shoffset)); //zman_tallit = 11 by default
$time_2 = date($time_format, $ts_zero+($hour*3600)+($min*60));
...
if (isset($data['zman_hide']) && $data['zman_hide-button']!="one") {
  print '<div class="textright">';
        print '<a href="#" class="hide zmanim-'.$ts_zero.'';
        //if (in_array("today",$hide_exclude)) print ' hiddenz';
        print '" rel="zmanim-'.$ts_zero.'">'.__("Show","zmanim").'</a>';
        print '<a href="#" class="hide zmanim-'.$ts_zero.'';
        //if (!in_array("today",$hide_exclude))
        print ' hiddenz';
        print '" rel="zmanim-'.$ts_zero.'">'.__("Hide","zmanim").'</a>';
   print '</div>';
}?>
</li></ul>
